...
function displayTransactions(transactions) {
    const historyTab = document.querySelector('.history__tab');
    historyTab.innerHTML = ''; // Очищаем предыдущие данные

    transactions.forEach((transaction, index) => {
        const isIncoming = transaction.to === username;
        const transactionClass = isIncoming ? 'incoming' : 'outcoming';
        const sign = isIncoming ? '+' : '-';

        const transactionHTML = `
            <div class="transcation ${transactionClass}">
                <div class="transaction__left">
                    <h4>From: <span>${isIncoming ? transaction.from : 'You'}</span></h4>
                    <h4><span>${transaction.time}</span> UTC+2</h4>
                    <h3 class="transaction___sum">${sign} <span>${transaction.amount}</span> <span>${transaction.currency}</span></h3>
                </div>
                <div class="transaction__right">
                    <h4>To: <span>${isIncoming ? 'You' : transaction.to}</span></h4>
                </div>
            </div>
            <hr>
        `;
